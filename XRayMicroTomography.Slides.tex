...
% Break marker: place \saveframenumber after the slide before the break.
% Writes the current frame number to \jobname.mkr; on the *next* run that
% value is read back and drawn as a tick on the progress bar.
% (Two compilations needed: first run writes the file, second reads it.)
\makeatletter
\newwrite\@markerout
\newcommand{\myframenumber}{-1}% -1 = no marker; overwritten by .mkr if present
...
% Footline: thin progress bar matching the UniBe header rule (1.36 mm tall)
% NOTE: beamer defaults \inserttotalframenumber to 1 (beamerbasemisc.sty), and the
% real value only arrives via the .nav file. On a clean build the ratio therefore
% grows without bound and \pgfmathsetlengthmacro overflows TeX's 16383.99pt limit
% at frame 36 (16383.99/455.24pt). Requiring >1 skips the bar on the first pass,
% and min(1,...) clamps it in case the count is ever stale.
\mode<beamer>{%
        ...
        \hbox{%
            \begin{beamercolorbox}[wd=\paperwidth,ht=1.36mm,dp=0pt]{}%
                \begin{tikzpicture}[overlay,remember picture]
                    \pgfmathsetlengthmacro{\halfheight}{0.68mm}% circle radius = half bar height
                    \ifnum\inserttotalframenumber>1
                        \pgfmathsetlengthmacro{\progressx}{%
                            min(1,\insertframenumber/\inserttotalframenumber)*\paperwidth}%
                        % circle scrubbing along the bar at the current frame
                        \draw[ubRed,fill=white] (\progressx,\halfheight) circle (\halfheight);
                        % break marker tick — only present after \saveframenumber has been called
                        \ifnum\myframenumber>0
                            \pgfmathsetlengthmacro{\markx}{%
                                min(1,\myframenumber/\inserttotalframenumber)*\paperwidth}%
            ...
            \only<3|handout:2>{%
                \begin{tikzpicture}[remember picture,overlay]
                    \node at (current page.center) {%
                        \mode<beamer>{\animategraphics[loop,autoplay,width=0.98\paperwidth,every=\everyframe]{24}{./movies/mouse_skull/mouse_skull}{000}{236}}
                        \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/mouse_skull/mouse_skull075}}
                    };
                \end{tikzpicture}%
            }%
        \end{column}
...
\begin{frame}
    ...
    \citeslide{./images/220923_Fundamentals_image_processing_2022_Witz-23}{https://ilias.unibe.ch/go/sess/2466919}{Fundamentals of Digital Image Processing}{Guillaume Witz}{23}
\end{frame}
