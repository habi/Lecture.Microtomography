...
\begin{frame}[label=current]{Examples}
	...
	\only<4>{%
		...
		\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
			...
			\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+46.296,\y) node [midway,above] {\shadowtext{\SI{500}{\micro\meter}}};
		\end{tikzpicture}%	
	}		
	\only<5|handout:0>{%		
		\animategraphics[autoplay,loop,height=\imheight,every=\everyframe]{25}{./movies/shell/Muschel_klein}{0000}{431}
	}
	\only<6>{%
