...
\newlength\imgwdtmp% helper temporary length for safe calculations
\newcommand{\setimagescalefrom}[2]{% #1 = width (length or macro), #2 = pixel width (numeric)
...
\progressbarheight=1.36mm
\addtobeamertemplate{footline}{}{%
    \mode<beamer>{%
        \begin{tikzpicture}[remember picture,overlay]
            % Bounding box for the footline
            \useasboundingbox (0,0) rectangle (\paperwidth,\progressbarheight);
            % Compute progress fraction safely
            \pgfmathparse{\insertframenumber/\inserttotalframenumber}
            \let\progressfraction\pgfmathresult
            % Convert fraction to TeX dimension relative to paper width
            \newdimen\progressx
            \setlength{\progressx}{\progressfraction\paperwidth}
            % Draw the progress circle
            \draw[draw=ubRed, fill=white] (\progressx, \progressbarheight/2) circle (\progressbarheight/2);
        \end{tikzpicture}%
    }%
}%
			...
			\only<3|handout:0>{%
				\lstinputlisting[linerange={2-4,6-7,14,16-18,26-29,32,36-38,41,45},
					  linebackgroundcolor={%
  						 ...
  						 \ifnum\value{lstnumber}=11 \color{ubRed!61.8}\fi				   						 \ifnum\value{lstnumber}=14 \color{ubRed!61.8}\fi
}
