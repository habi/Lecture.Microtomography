...
% Load "several" bibliographies (no matter where we run this thing)
% https://tex.stackexchange.com/a/295142/828
\addbibresource{../../../Documents/library.bib} % FastSSD or Windows (on Linux we generated a 'Document' folder at the correct level and `ln -s ~/P/Documents/library.bib .` the correct file to it)
...
\begin{frame}{Examples}
	...
	\only<4|handout:3>{%
		...
		\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
			...
			\end{scope}
			%\spy [red] on (87,10) in node at (0,0) [anchor=north west];
	...
	\only<5|handout:0>{%
		\animategraphics[autoplay,palindrome,width=0.618\linewidth,every=\everyframe]{24}{./movies/diancta/Diancta_rec00000}{666}{921}%
	}%
	\only<6|handout:4>{%
