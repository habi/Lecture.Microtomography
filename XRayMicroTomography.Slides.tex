...
\mode<beamer>{\newcommand{\everyframe}{75}} % use only every nth frame for the animations
\mode<handout>{\newcommand{\everyframe}{75}} %kill animations for `handout'
...
\section{Imaging}
\begin{frame}[label=current]{Wavelength \& Scale}
	\centering
	\only<1|handout:1>{%
...
\begin{frame}[label=current]{History}
	\begin{columns}
		...
		\begin{column}{0.48\linewidth}
			...
			\includegraphics<4|handout:3>[width=\imwidth]{./images/History_Generation3}%
...
\begin{frame}
	...
	\begin{tikzpicture}[remember picture,overlay]%
		\node at (current page.center){%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./movies/fishhead/head-without-gills0}{001}{160}%
			};%
	\end{tikzpicture}%
\end{frame}
...
\begin{frame}
	...
	\begin{tikzpicture}[remember picture,overlay]%
		\node at (current page.center){%
			\animategraphics[autoplay,width=\paperwidth,every=\everyframe]{24}{./movies/fishhead/head-without-gills0}{161}{481}%
			};%
	\end{tikzpicture}%
\end{frame}

\renewcommand{\imwidth}{\columnwidth}
\begin{frame}[label=current]{How?}
	\begin{columns}
	...
	\begin{column}{0.49\linewidth}
		...
		\only<3|handout:3>{\centering\Huge\faFish:\xspace\faRuler\xspace\&\xspace\faWeight}
		\renewcommand{\imwidth}{0.5\columnwidth}%
		\only<4|handout:4>{%
			\pgfmathsetlength{\imagewidth}{\imwidth}%
		...
		\includegraphics<7>[width=\imwidth]{./images/CTAn}%
		\only<8>{%
...
\begin{frame}[label=current]{Gill complexity}
	\centering
	\pgfmathsetlength{\imagewidth}{0.618\linewidth}%
	\pgfmathsetlength{\imagescale}{\imagewidth/2163}%
	\def\x{1730}% scalebar-x starting at golden ratio of image width of 2163px = 1337
	...
	\only<4>{%
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center){%
				\animategraphics[palindrome,autoplay,width=\paperwidth,every=\everyframe]{25}{./movies/complexity/control03_thresholded_0}{001}{256}%
				};%
		\end{tikzpicture}%
		}%
