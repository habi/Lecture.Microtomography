...
% Define complementary colors to ubRed, from https://www.canva.com/colors/color-wheel/
\definecolor{ubRedComplementary}{HTML}{00E6B8}
...
\progressbar@pbwd=0.618\paperwidth%
...
\def\progressbar@progressbar{%
	...
	\divide\progressbar@tmpdim by \progressbar@tmpcountb%
		\mode<beamer>{%
			...
			\hfill%
			git.io/fjpP7\xspace|%
			\xspace\href{https://github.com/habi/Lecture.Microtomography/commit/\gitHash}{\gitAbbrevHash}\xspace|%
			\xspace\insertframenumber/\inserttotalframenumber%
			}%
\mode<handout>{%
	Handout for \href{https://github.com/habi/Lecture.Microtomography}{git.io/fjpP7}\xspace|%
	\xspace Generated on \DTMnow, from version \href{https://github.com/habi/Lecture.Microtomography/commit/\gitHash}{\gitAbbrevHash}\xspace|%
	\xspace Page\xspace\insertframenumber/\inserttotalframenumber%
	}%
\vspace{0.5ex}%
}%
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				\item \href{https://ilias.unibe.ch/goto_ilias3_unibe_sess_2774501.html}{\emph{Light Sheet Microscopy}} by Nadia Mercader Huber
				\item X-ray imaging
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item<2-|handout:2-> CT scanner generations
				\begin{itemize}
					\item<2-|handout:2-> First generation
					\item<3-|handout:3-> Second generation
					\item<4|handout:4> Third generation
				\end{itemize}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			...
			\includegraphics<2|handout:2>[height=\imageheight]{./images/History_Generation1}%
			\only<2|handout:2>{\sourcecite{Hsieh2003}{Figure 1.12}}%
			\includegraphics<3|handout:3>[height=\imageheight]{./images/History_Generation2}%
			\only<3|handout:3>{\sourcecite{Hsieh2003}{Figure 1.13}}%
			\includegraphics<4|handout:4>[height=\imageheight]{./images/History_Generation3}%
...
\begin{frame}
	...
	\begin{columns}
		...
		\end{column}
	\end{columns}
	\note<1|handout:1>{The human head scan was downloaded from the \href{https://www.cancerimagingarchive.net/nbia-search/?saved-cart=nbia-76761575299081509}{Cancer Imaging Archive}.
...
\begin{frame}
		...
		% Then `pdftoppm -png Fundamentals_image_processing_2024.pdf Fundamentals_image_processing_2024` and copy relevant slide images to here
		\only<1|handout:0>{%
			\includegraphics[height=\imageheight]{./images/Fundamentals_image_processing_2024-21.png}%
			\sourcelink{ilias.unibe.ch/goto_ilias3_unibe_sess_3022928.html}{\emph{Fundamentals of Digital Image Processing} by Guillaume Witz}{Slide 21}%
		}%
		\only<2|handout:1>{
			\includegraphics[height=\imageheight]{./images/Fundamentals_image_processing_2024-23.png}%
			\sourcelink{ilias.unibe.ch/goto_ilias3_unibe_sess_3022928.html}{\emph{Fundamentals of Digital Image Processing} by Guillaume Witz}{Slide 23}%
		}%
\end{frame}
...
\begin{frame}
	...
	\centering%
	\only<1|handout:1>{%
		\pgfmathsetlength{\imagewidth}{0.9\textwidth}%
		\pgfmathsetlength{\imagescale}{\imagewidth/3295}%
		\def\x{2036}% scalebar-x starting at golden ratio of image width of 3295px = 2036
		\def\y{1343}% scalebar-y at 90% of image height of 1492px = 1343
		\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
	...
	\pgfmathsetlength{\imagewidth}{0.5\textwidth}%
	...
	\only<5|handout:0>{%
		\animategraphics[autoplay,palindrome,width=\imagewidth,every=\everyframe]{24}{./movies/diancta/Diancta_rec00000}{666}{904}%
		\sourcecite{Bochud2021}{\emph{Diancta phoenīx}}%
	}%
	\only<6|handout:4>{%
		\pgfmathsetlength{\imagewidth}{0.8\textwidth}%
...
\begin{frame}
	...
	\begin{itemize}%
		\item<1-2|handout:1> Numbers instead of just pretty images%
		...
		\begin{itemize}%
			\item \cite{Haberthuer2021}: \href{https://bmcoralhealth.biomedcentral.com/articles/10.1186/s12903-021-01551-x}{doi.org/gjpw2d}%
			\item \cite{Wolf2021}: \href{https://www.nature.com/articles/s41598-021-00758-w}{doi.org/g7r8}%
		\end{itemize}%
	\end{itemize}%
	\only<1|handout:0>{%
		\begin{tikzpicture}[remember picture,overlay]%
...
\begin{frame}
	...
	\begin{columns}%
		...
		\begin{column}{0.5\textwidth}%
			\only<1|handout:1>{%
				\centering%
				% Redefine image width, because of the big square bucket image
				\renewcommand{\imagewidth}{0.8\textwidth}%
			...
			% \only<7|handout:7>{%
				...
				\mode<handout>{%
					\includegraphics[width=\imagewidth]{./movies/coder/coder-65}%
					\source{gph.is/2nqkple}{}%
					}%
				}%
			\only<7|handout:7>{%
...
\begin{frame}[shrink=25]
	...
	\begin{table}[]%
		...
		\begin{tabular}{llcSS}%
			...
			\multicolumn{2}{c}{Roots} & RCC & \# & \%\\%
			\midrule%
			\multicolumn{2}{c}{\multirow{9}{*}{Single (N=98)}}	& 1--1--1/1 & \cellcolor{ubRed!61.8}73 & \cellcolor{ubRed!61.8}74.5\\%
			\multicolumn{2}{l}{}																& 1--1--1/2 & \cellcolor{ubRed!61.8}14 & \cellcolor{ubRed!61.8}14.3\\%
			\multicolumn{2}{l}{}																& 1--1--1/3 & 1 & 1.0\\%
			\multicolumn{2}{l}{}																& 1--1--1/4 & \cellcolor{ubRed!61.8}2 & \cellcolor{ubRed!61.8}2.1\\%
			\multicolumn{2}{l}{}																& 1--1--2/1 & 1 & 1.0\\%
			\multicolumn{2}{l}{}																& 1--2--1/1 & \cellcolor{ubRed!61.8}4 & \cellcolor{ubRed!61.8}4.1\\%
			\multicolumn{2}{l}{}																& 1--2--1/2 & 1 & 1.0\\%
			\multicolumn{2}{l}{}																& 1--2--2/2 & 1 & 1.0\\%
			\multicolumn{2}{l}{}																& 2--3--1/1 & 1 & 1.0\\%
			\midrule%
...
\begin{frame}
	...
	\frametitle{Results of root canal space extraction}%
		...
		\renewcommand{\imagewidth}{0.14\textwidth}% 720/2008
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item The \LaTeX\xspace code is automatically compiled with a \href{https://github.com/actions}{GitHub action}%\footnote{Details on how this works are specified in a \href{https://github.com/habi/latex-test/}{small test repository here: git.io/JeOOj}}%
			to the \href{https://habi.github.io/Lecture.Microtomography/XRayMicroTomography.Handout.pdf}{(handout) PDF linked on ILIAS (git.io/JeQxO)}
		\end{itemize}
