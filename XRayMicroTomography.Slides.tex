			...
			\only<3|handout:2>{%
				\begin{tikzpicture}[remember picture,overlay]%
					\node at (current page.center){%
						\mode<beamer>{\animategraphics[palindrome,autoplay,width=\paperwidth,every=\everyframe]{25}{./movies/mouse_skull/mouse_skull}{000}{470}}
...
\begin{frame}[label=current]{History}
	\begin{columns}
		\begin{column}{0.49\linewidth}
			\begin{itemize}
				\item<1-|handout:1-> Extensive history is found in~\cite{Cormack1963,Hsieh2003}
...
\begin{frame}[label=current]{Composition of biological tissues}
	\centering
	\only<1>{%
		\begin{table}
...
\begin{frame}[label=current]{Examples}
	\centering
	\only<1|handout:1>{%
	...
	\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
		...
		\only<2-3|handout:2>{%
			\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+200,\y) node [midway,above] {\shadowtext{\SI{1}{\milli\meter}}};
		}
	\end{tikzpicture}%
	\only<4|handout:3>{%
		...
		\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
			\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./movies/shell/Muschel_klein0216}};
			...
			\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+92.59,\y) node [midway,above] {\shadowtext{\SI{1}{\milli\meter}}};
		\end{tikzpicture}%
	}
	\only<5|handout:0>{%
		\animategraphics[autoplay,loop,width=0.618\linewidth,every=\everyframe]{25}{./movies/shell/Muschel_klein}{0000}{431}
	}
	\only<6|handout:4>{%
		\pgfmathsetlength{\imagewidth}{0.55\linewidth}%
		\pgfmathsetlength{\imagescale}{\imagewidth/708}%
		\def\x{438+200}% scalebar-x starting at golden ratio of image width of 708px = 438
		...
		\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
			...
			\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+46.3,\y) node [midway,above] {\shadowtext{\SI{1}{\milli\meter}}};
		\end{tikzpicture}%
	}
\end{frame}
...
\begin{frame}{Visualization}
	\begin{tikzpicture}[remember picture,overlay]%
	\node at (current page.center){%
		\mode<beamer>{\animategraphics[autoplay,palindrome,width=\paperwidth,every=\everyframe]{25}{./movies/scan/visualization/lung}{000}{473}}%
...
\begin{frame}{What to use?}
	\begin{itemize}
		...
		\item \faPython in Jupyter~\cite{Kluyver2016}
...
\subsection{Example of a full study}
\begin{frame}
