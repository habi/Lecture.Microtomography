...
\subtitle{\href{https://ilias.unibe.ch/go/sess/3728317}{485018-HS2026-0: Advanced Course II Ultraprecision Engineering}}
...
\begin{frame}
    ...
    \mode<beamer>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node[anchor=east,xshift=-5mm,yshift=-10mm] at (current page.east |-
            current page.center)
            {%
    ...
    \begin{columns}%
        ...
        \begin{column}{0.62\textwidth}%
            \begin{itemize}%
                ...
                \item<1-|handout:1-> Results in three-dimensional images \uncover<2-|handout:2>{with calibrated and isotropic voxels at \unit{\micro\meter} resolution}
                    \only<1|handout:1>{
                \item Covers a large range of sample sizes}
                    \only<2-|handout:2>{
                \item Covers a \emph{very} large range of sample sizes}
...
\begin{frame}
    ...
    \only<2|handout:2>{%
        \citeslide{./images/MIC-AM_techniques}{https://ilias.unibe.ch/ilias.php?baseClass=ilrepositorygui&cmd=infoScreenGoto&ref_id=1401591}{Introduction to Microscopy}{Stefan Tschanz}{1}%
    }%
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.49\linewidth}
            \begin{itemize}
                ...
                \item<2-> \citeyear{Cormack1963}:~\citeauthor{Cormack1963} used a collimated \ce{^{60}Co} source and a Geiger counter as a detector~\cite{Cormack1963}%
                \item<3-> \citeyear{Hounsfield1976a}~\citeauthor{Hounsfield1976a} worked on first clinical scanner~\cite{Hounsfield1976a}%
                \item<5-> Nobel Prize in 1979, jointly for Allan Cormack and Godfrey Hounsfield
                \item<6-> CT scanner generations
                    \begin{itemize}
                        \item<6-> First generation
                        \item<7-> Second generation
                        \item<8> Third generation
                    \end{itemize}
            \end{itemize}
        \end{column}
        \begin{column}{0.49\linewidth}
            \centering%
            %\begin{overlayarea}{\linewidth}{\imageareaheight}
            \includegraphics<1>[height=\imageheight]{./images/First_medical_X-ray_by_Wilhelm_Röntgen_of_his_wife_Anna_Bertha_Ludwig's_hand_-_18951222}%
            \only<1>{\source{w.wiki/BHAN}{\ccPublicDomain}}%
            \includegraphics<2>[height=\imageheight]{./images/nbla_294282}%
            \only<2>{\sourcelink{https://repository.aip.org/node/121569}{AIP Emilio Segrè Visual Archives, Physics Today Collection}{}}%
            \includegraphics<3>[width=\imagewidth]{./images/large_b990545.jpg}%
            \only<3>{\sourcelink{https://collection.sciencemuseumgroup.org.uk/objects/co134790/}{Science Museum Group Collection Online}{\ccbync}}%
            \only<4>{%
            ...
            \only<4>{\sourcecite{Beckmann2006}{Figure 5}}%
            \includegraphics<5>[height=\imageheight]{./images/US3940625-drawings-page-2}%
            \only<5>{\sourcecite{US3940625A}{p.~2}}%
            \includegraphics<6>[width=\imagewidth]{./images/History_Generation1}%
            \only<6>{\sourcecite{Hsieh2003}{Figure 1.12}}%
            \includegraphics<7>[width=\imagewidth]{./images/History_Generation2}%
            \only<7>{\sourcecite{Hsieh2003}{Figure 1.13}}%
            \includegraphics<8>[width=\imagewidth]{./images/History_Generation3}%
            \only<8>{\sourcecite{Hsieh2003}{Figure 1.14}}%
            %\end{overlayarea}
        \end{column}
    \end{columns}
    ...
    \begin{itemize}
        \item \textquote[\cite{xrayphysics}]{X-rays interact with tissue in 2 main ways: photoelectric effect and Compton scatter.
            ...
            \begin{itemize}
                ...
                \item Compton scattering is one of the principal forms of photon interaction and is directly proportional to the (electron \& physical) density of the material.
            ...
            \end{itemize}
        \item Low X-ray energy and high-\(Z\) materials tend to produce strong contrast
        \item X-ray penetration decreases exponentially with sample thickness (Beer--Lambert law,~\cite{wiki:beer-lambert}): \(I(t) = I_0 \, e^{-\alpha z}\)
    \end{itemize}
\end{frame}

\begin{frame}
    \frametitle{Composition of Biological Tissues}
    \captionof*{table}{Tissue: content by mass percentage}
    \centering
    \only<1|handout:0>{%
        \pgfplotstabletypeset{./tables/tissue-composition.csv}%
    }%
    \only<2|handout:1>{%
...
\begin{frame}
    ...
    \note<1|handout:1>{The human head scan was downloaded from the \href{https://www.cancerimagingarchive.net}{Cancer Imaging Archive}.
        We loaded the DICOM slices in Fiji, resliced it to show it from the side and then used it to generate an MIP\@.
...
\begin{frame}
    ...
    % In 2025, the content of the relevant slides has not changed, it's just different page numbers :)
    \only<1|handout:0>{\citeslide{./images/Fundamentals_image_processing_2024-021}{https://ilias.unibe.ch/go/sess/3400760}{Fundamentals of Digital Image Processing}{Guillaume Witz}{22}}
    \only<2|handout:1>{\citeslide{./images/Fundamentals_image_processing_2024-023}{https://ilias.unibe.ch/go/sess/3400760}{Fundamentals of Digital Image Processing}{Guillaume Witz}{24}}
\end{frame}
...
\begin{frame}
    \frametitle{What Is Happening in a CT vs.\ a \uct?}
    \begin{columns}
...
\section[A Scan, from Getting Started to Nice Image]{A Scan, from \emph{Getting Started} to \emph{Nice Image}}
...
\begin{frame}[allowframebreaks]
    ...
    % Movie downloaded from https://sites.google.com/a/fulbrightmail.org/kesnersmedicalphysics/home/education/3d-image-reconstruction-explained-with-animated-gifs, slightly cropped in ImageJ, saved out and PNG images and piped through oxipng
...
\begin{frame}[allowframebreaks]
    ...
    \renewcommand{\imageheight}{0.68\paperheight}%
    ...
    \mode<handout>{\includegraphics[width=\imagewidth]{./movies/scan/sinogram/TOMO_FBP_gif_CT_signed0055}\sourceqr{https://sites.google.com/a/fulbrightmail.org/kesnersmedicalphysics/home/education/3d-image-reconstruction-explained-with-animated-gifs}}
    \renewcommand{\imageheight}{\standardimageheight}
    \framebreak
...
% After this frame, it's a good time for a break in the UPE lecture, so save us the frame number after it
\begin{frame}[allowframebreaks]
...
\begin{frame}
    ...
    \centering%
    \only<1|handout:0>{\includegraphics[height=\imageheight]{./images/2214}}%
    \only<2|handout:0>{\includegraphics[height=\imageheight]{./images/2214/room}}%
    \only<3|handout:0>{\includegraphics[height=\imageheight]{./images/2214/scanning_compartement}}%
    \only<4|handout:1>{\includegraphics[height=\imageheight]{./images/2214/ControlSoftware01}}%
\end{frame}

\begin{frame}
    \frametitle{Demo: Reconstruction}
    \centering%
    \only<1|handout:1>{\includegraphics[height=\imageheight]{./images/2214/NRecon_proj}}%
    \only<2|handout:2>{\includegraphics[height=\imageheight]{./images/2214/NRecon_rec}}%
\end{frame}

\begin{frame}
    \frametitle{Demo: Visualization}
    \centering%
    \includegraphics[height=\imageheight]{./images/2214/Dragonfly}%
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Two publications:
            \begin{itemize}
                \item Methodological~\cite{Haberthuer2021}, \href{https://bmcoralhealth.biomedcentral.com/articles/10.1186/s12903-021-01551-x}{BMC Oral Health, doi.org/gjpw2d}
    ...
    \only<1|handout:1>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) {%
                \mode<beamer>{\animategraphics[autoplay,loop,width=0.98\paperwidth,every=\everyframe]{24}{./movies/tooth045/full/image0}{000}{474}}%
                \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/tooth045/full/image0237}}%
            };
        \end{tikzpicture}%
    }%
\end{frame}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            ...
            \renewcommand{\imageheight}{0.5\textheight}%
            \only<8|handout:8>{%
                \centering%
...
\begin{frame}
    ...
    \only<1|handout:1>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) [shift={(0,-2.9mm)}]{%
                \mode<beamer>{\animategraphics[autoplay,loop,width=0.98\paperwidth,every=\everyframe]{24}{./movies/tooth045/full/image0}{000}{474}}%
                \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/tooth045/full/image0000}}%
            };
        \end{tikzpicture}%
    }%
    \only<2|handout:2>{%
    ...
    \only<3|handout:3>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) [shift={(0,-2.9mm)}]{%
                \mode<beamer>{\animategraphics[autoplay,loop,width=0.98\paperwidth,every=\everyframe]{24}{./movies/tooth045/full-slices/image0}{000}{466}}%
                \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/tooth045/full-slices/image0000}}%
            };
        \end{tikzpicture}%
    }%
\end{frame}
...
\begin{frame}
    ...
    \only<1|handout:0>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) [shift={(0,-2.9mm)}]{%
                \mode<beamer>{\animategraphics[autoplay,loop,width=0.98\paperwidth,every=\everyframe]{24}{./movies/tooth045/transparent-slices/image0}{000}{473}}%
                \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/tooth045/transparent-slices/image0000}}%
            };
        \end{tikzpicture}%
    }%
    \only<2|handout:1>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) [shift={(0,-2.9mm)}]{%
                \mode<beamer>{\animategraphics[autoplay,loop,width=0.98\paperwidth,every=\everyframe]{24}{./movies/tooth045/transparent-slices-rcs/image0}{000}{457}}%
                \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/tooth045/transparent-slices-rcs/image0000}}%
            };
        \end{tikzpicture}%
    }%
    \only<3|handout:0>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) [shift={(0,-2.9mm)}]{%
                \mode<beamer>{\animategraphics[autoplay,loop,width=0.98\paperwidth,every=\everyframe]{24}{./movies/tooth045/rcs/image0}{000}{413}}%
                \mode<handout>{\includegraphics[width=0.98\paperwidth]{./movies/tooth045/rcs/image0000}}%
            };
        \end{tikzpicture}%
    }%
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Scales effortlessly: analysing \num{10} or \num{10000} teeth takes the same manual effort
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            ...
            \begin{itemize}
                ...
                \item 375 scans in total
                    \begin{itemize}
                        \item Voxel sizes from \qtyrange{3.5}{50}{\micro\meter}
        ...
        \begin{column}{0.5\linewidth}
            ...
            \only<2|handout:0>{%
                \lstinputlisting[linerange={2-4,6-7,14,16-18,26-29,32,36-38,41,45}]{./movies/EAWAG/104016/head/proj/104016.log}
            }%
            \only<3|handout:0>{%
                \lstinputlisting[
                    linerange={2-4,6-7,14,16-18,26-29,32,36-38,41,45},
                    linebackgroundcolor={%
                        \ifnum\value{lstnumber}=1 \color{ubRed!61.8}\fi
                        \ifnum\value{lstnumber}=5 \color{ubRed!61.8}\fi
                        \ifnum\value{lstnumber}=10 \color{ubRed!61.8}\fi
                        \ifnum\value{lstnumber}=11 \color{ubRed!61.8}\fi
                        \ifnum\value{lstnumber}=14 \color{ubRed!61.8}\fi
                    }%
                ]{./movies/EAWAG/104016/head/proj/104016.log}
            }%
            \only<4|handout:0>{%
                \lstinputlisting[
                    linerange={2-4,6-7,14,17-19,26-29,31,36-38,40,45},
                    linebackgroundcolor={%
                        \ifnum\value{lstnumber}=1 \color{ubRed!61.8}\fi
                        \ifnum\value{lstnumber}=5 \color{ubRed!61.8}\fi
                        \ifnum\value{lstnumber}=10 \color{ubRed!61.8}\fi
                ...
                ]{./movies/EAWAG/104016/pharynx/proj/104016.log}
            }%
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \only<1|handout:1>{%
        ...
        \begin{tikzpicture}[x=\imagescale,y=-\imagescale]%
            \node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./movies/EAWAG/animation.head/104016.head.rec.animation.0000.jpg}};%
    ...
    \only<2|handout:2>{%
        % Show PJ
        % Since the animation is too slow, we specifically set it double speed here
        \mode<beamer>{\animategraphics[autoplay,width=\imagewidth,every=\numexpr2*\everyframe\relax]{24}{./movies/EAWAG/animation.head/104016.head.rec.animation.0}{000}{480}}%
    ...
    \only<3|handout:0>{%
        ...
        % Since the animation is too slow, we specifically set it double speed here
        \animategraphics[autoplay,width=\imagewidth,every=\numexpr2*\everyframe\relax]{24}{./movies/EAWAG/animation.head/104016.head.rec.animation.}{0481}{0960}%
    }%
    \only<4|handout:0>{%
        % Zoom to otolith
        % Since the animation is too slow, we specifically set it double speed here
        \animategraphics[autoplay,width=\imagewidth,every=\numexpr2*\everyframe\relax]{24}{./movies/EAWAG/animation.head/104016.head.rec.animation.}{0961}{1440}%
    }%
    \only<5|handout:0>{%
        % Move to PJ
        % Since the animation is too slow, we specifically set it double speed here
        \animategraphics[autoplay,width=\imagewidth,every=\numexpr2*\everyframe\relax]{24}{./movies/EAWAG/animation.head/104016.head.rec.animation.}{1441}{1920}%
    }%
...
\begin{frame}
    ...
    \only<2|handout:1>{%
        ...
        \def\y{648}% scalebar-y at 90% of image height of 720px = 648
        \begin{tikzpicture}[x=\imagescale,y=-\imagescale]%
            \node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./movies/EAWAG/animation.pj/104016.pj.animation.0000.jpg}};%
    ...
    \only<3|handout:0>{%
        % Since the animation is too slow, we specifically set it faster speed here
        \animategraphics[autoplay,width=\imagewidth,every=\numexpr4*\everyframe\relax]{24}{./movies/EAWAG/animation.pj/104016.pj.animation.0}{000}{700}%
    }%
    \only<4|handout:0>{%
        % Since the animation is too slow, we specifically set it double speed here
        \animategraphics[autoplay,width=\imagewidth,every=\numexpr4*\everyframe\relax]{24}{./movies/EAWAG/animation.pj/104016.pj.animation.}{0701}{1440}%
    }%
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item<2-> What would you like to look at with \uct?
        \item<3-> \href{https://mannerofspeaking.org/2019/12/29/what-questions-do-you-have-for-me/}{What questions do you have for me?}%
    \end{itemize}%
    \only<4|handout:0>{%
        \begin{tikzpicture}[remember picture,overlay]
            \node at (current page.center) {%
                \animategraphics[loop,autoplay,width=0.98\paperwidth,every=\everyframe]{24}{./movies/mouse_skull/mouse_skull}{000}{236}%
            };
        \end{tikzpicture}%
    }%
\end{frame}
