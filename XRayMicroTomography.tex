...
\usepackage{tikz}
	...
	\tikzexternalize[prefix=animations/] % externalize tikz figures into pdf frames, speeds up compilation
\usepackage{pgfplots}
...
\begin{frame}{History}
	\begin{columns}
		...
		\begin{column}{0.48\linewidth}
			\centering
			\includegraphics<2>[width=\imwidth]{./images/{{History.1Generation}}}%
			\only<2>{\sourcecite{Hsieh2003}{Figure 1.12}}%
			\includegraphics<3>[width=\imwidth]{./images/{{History.2Generation}}}%
			\only<3>{\sourcecite{Hsieh2003}{Figure 1.13}}%
			\includegraphics<4>[width=\imwidth]{./images/{{History.3Generation}}}%
...
\begin{tikzpicture}[x=\imagescale,y=-\imagescale, spy using outlines={rectangle, magnification=\mag, size=\size, connect spies}]
	...
	\draw[|-|,thick] (\x+\shadow,\y+\shadow) -- (\x+102.4+\shadow,\y+\shadow) node [midway, above] {\SI{5}{\centi\meter}};
	\draw[|-|,white,thick] (\x,\y) -- (\x+102.4,\y) node [midway,above] {\SI{5}{\centi\meter}};
	\draw[color=red, anchor=south west] (0,401) node [fill=white, semitransparent] {Legend} node {Legend};
\end{tikzpicture}%
...
\begin{tikzpicture}[x=\imagescale,y=-\imagescale, spy using outlines={rectangle, magnification=\mag, size=\size, connect spies}]
	\begin{scope}
		\clip (0,0) rectangle (3295,1492);
		%\clip (1647.5,746.0) circle (746.0);
		\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./images/comparison/{{MAX_mouse}}}};
	\end{scope}
	%\spy [red] on (2995,1192) in node at (0,0) [anchor=north west];
	% 3295.000px = 26.2282mm -> 100px = 796.000um -> 62.814px = 500um, 12.563px = 100um
	\draw[|-|,blue,thick] (0,746) -- (3295,746) node [sloped,midway,above,fill=white,semitransparent,text opacity=1] {\SI{26.2282}{\milli\meter} (3295px) asdfasdfasdfasdfasdf!};
	\draw[|-|,thick] (\x+\shadow,\y+\shadow) -- (\x+62.814+\shadow,\y+\shadow) node [midway, above] {\SI{500}{\micro\meter}};
	\draw[|-|,white,thick] (\x,\y) -- (\x+62.814,\y) node [midway,above] {\SI{500}{\micro\meter}};
	\draw[color=red, anchor=south west] (0,1492) node [fill=white, semitransparent] {Legend} node {Legend};
\end{tikzpicture}%
%			}
%			\only<2<{%
%				\pgfmathsetlength{\imagewidth}{0.0163\imwidth}% 7.96 um / 488um
%				\pgfmathsetlength{\imagescale}{\imagewidth/765}%
%				\def\x{473}% scalebar-x starting at golden ratio of image width of 765px = 473
%				\def\y{356}% scalebar-y at 90% of image height of 396px = 356
%				\def\shadow{4}% shadow parameter for scalebar
%				\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
%					\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./images/comparison/{{MAX_mouse}}}};
%					% 765px = 6.0893999999999995mm > 100px = 796um > 63px = 500um, 13px = 100um
%					%\draw[|-|,blue,thick] (0,198) -- (765,198) node [sloped,midway,above,fill=white,semitransparent,text opacity=1] {\SI{6.0893999999999995}{\milli\meter} (765px) TEMPORARY!};
%					\draw[|-|,thick] (\x+\shadow,\y+\shadow) -- (\x+63+\shadow,\y+\shadow) node [midway, above] {\SI{500}{\micro\meter}};
%					\draw[|-|,white,thick] (\x,\y) -- (\x+63,\y) node [midway,above] {\SI{500}{\micro\meter}};
%					%\draw[color=red, anchor=south west] (0,396) node [fill=white, semitransparent] {Legend} node {Legend};
%			\end{tikzpicture}%
%			}
		\end{column}
		\hfill
