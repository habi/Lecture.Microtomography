...
\title{Self-Test Questions for the Lecture on X-Ray Microtomography}
\author{David Haberthür}
\date{\today\xspace| v.~\gitAbbrevHash}

% Helvetica font family default
...
\begin{questions}
    ...
    \question Describe what happens with the X-rays once they are emitted from the X-ray source of a desktop \mct or large human CT\@, until we get a projection image on disk.
    Describe all interactions with the sample and the detector along the way.

    \question In a desktop \uct the researcher can change the voxel size of the image acquisition nearly continuously (within a certain range).
    This is not possible in a large CT which is used for imaging humans in a hospital.
    Why is this so?

    \question A researcher wants to image a sample approximately \qty{30}{\centi\meter} in size with a very small voxel size (let's say below \qty{1}{\micro\meter}).
    ...
    Compare the contrast of the skull bones, which are visible in both images; it looks quite similar.
    Both samples have been scanned on different machines and with settings that result in slightly different voxel sizes, but this is \emph{not} the cause\ldots
    ...
    \begin{figure}[h]
        ...
        \begin{subfigure}{0.495\textwidth}
            ...
            \end{tikzpicture}%
            \caption{
                \emph{Mouse 1265}, which you might remember from the visualization in the introduction section of the lecture.%
        ...
        \begin{subfigure}{0.495\textwidth}
            ...
            \label{fig:5158}%
        \end{subfigure}%
        \caption{\href{https://en.wikipedia.org/wiki/Maximum_intensity_projection}{MIP}s from a region of approximately \qty{500}{\micro\meter} around the sagittal centerline of two different mouse heads, scanned on two different desktop \uct scanners.}%
    \end{figure}
